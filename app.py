...
# Load the Model along with Scaler
...
age = st.number_input("Age", 18, 100, 45) # range 18-100, default=45
...
totChol = st.number_input("Total Cholestrol",100, 700, 220)
sysBP = st.number_input("Systolic BP", 70, 300, 120)
diaBP = st.number_input("Diastolic BP", 40, 200, 80)
BMI = st.number_input("BMI", 10.0, 70.0, 25.0)
heartRate = st.slider("Heart rate", 30, 200, 75)
glucose = st.slider("Glucose", 40, 500, 80)
#-----------
# Prediction
#-----------
...
## ---- Scale Input ----
    ...
    st.markdown('---')
    if prediction[0] == 1:
        st.error("High rick of Coronary Heart Disease")
    else:
